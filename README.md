...
* **Controllo condizioni meteo avverse:** Per questo scenario sono stati realizzati in selenium i file di test *TestIsNotChargingStateTemperature* e *TestIsNotChargingStateWind*. In questi test vengono simulate delle condizioni meteo avverse, quali rispettivamente la presenza di un clima troppo freddo o vento troppo veloce. Tali situazioni vengono realizzate premendo più volte i pulsanti per incrementare la velocità del vento o diminuire la temperatura fino al raggiungimento della condizione climatica pericolosa. A questo punto viene controllata la textArea del server di manutenzione che presenti un solo messaggio contenente il dato *isCharging:false*. Entrmabi i test procedono poi a far tornare il meteo in buone condizioni ed a vberificare che lo stato della stazione riporti che si sta ricaricando nuovamente.
